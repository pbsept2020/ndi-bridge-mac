...
| Phase 3: Buffer | 🎯 NOW | Buffer configurable pour diffusion stable |
| Phase 4: WAN | ⏳ TODO | STUN/TURN, NAT traversal |
| Phase 5: UI | ⏳ TODO | SwiftUI app |
| Phase 6: NDI\|HX | 🔒 BLOCKED | Nécessite NDI Advanced SDK (voir FUTURE_OPTIMIZATIONS.md) |
...
## 🎯 PHASE 3 : BUFFER CONFIGURABLE (EN COURS)

**Objectif:** Permettre un délai configurable pour diffusion LAN stable.
**Use case:** Universités, institutions avec diffusion multi-salles - le buffer absorbe le jitter réseau.
...
- Timer 1ms pour émission des frames prêtes

**Fichiers à modifier:**
- `main.swift` - Parser `--buffer <ms>`
- `JoinMode.swift` - Config buffer, intégration FrameBuffer
- **Nouveau:** `Common/FrameBuffer.swift` - Ring buffer thread-safe avec timestamps
...
- Sélection des sources NDI
- Configuration host/join
- Statistiques temps réel

---

## 🔒 PHASE 6 : NDI|HX (BLOQUÉ)

⚠️ **Nécessite NDI Advanced SDK** (non disponible gratuitement)

Voir `Docs/FUTURE_OPTIMIZATIONS.md` section 5 pour :
- Options d'obtention du SDK Advanced
- Use cases visés (diffusion 50+ endpoints)
